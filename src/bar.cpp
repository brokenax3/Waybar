...
static constexpr const char* MIN_HEIGHT_MSG =
    "Requested height: {} is less than the minimum height: {} required by the "
    "modules";

static constexpr const char* MIN_WIDTH_MSG =
    "Requested width: {} is less than the minimum width: {} required by the "
    "modules";
 ...
 * Assumes that all the values in the object are deserializable to the same
 * type.
 */
template <typename Key, typename Value,
          typename = std::enable_if_t<
              std::is_convertible<std::string_view, Key>::value>>
...
struct GLSSurfaceImpl : public BarSurface, public sigc::trackable {
    GLSSurfaceImpl(Gtk::Window& window, struct waybar_output& output)
        ...
        window.signal_map_event().connect_notify(
            sigc::mem_fun(*this, &GLSSurfaceImpl::onMap));
    ...
    void setMargins(const struct bar_margins& margins) override {
        gtk_layer_set_margin(window_.gobj(), GTK_LAYER_SHELL_EDGE_LEFT,
                             margins.left);
        gtk_layer_set_margin(window_.gobj(), GTK_LAYER_SHELL_EDGE_RIGHT,
                             margins.right);
        gtk_layer_set_margin(window_.gobj(), GTK_LAYER_SHELL_EDGE_TOP,
                             margins.top);
        gtk_layer_set_margin(window_.gobj(), GTK_LAYER_SHELL_EDGE_BOTTOM,
                             margins.bottom);
    }
    ...
    void setPosition(const std::string_view& position) override {
        ...
        if (vertical_ && height_ > 1) {
            gtk_layer_set_anchor(window_.gobj(), GTK_LAYER_SHELL_EDGE_BOTTOM,
                                 false);
            gtk_layer_set_anchor(window_.gobj(), GTK_LAYER_SHELL_EDGE_TOP,
                                 false);
        } else if (!vertical_ && width_ > 1) {
            gtk_layer_set_anchor(window_.gobj(), GTK_LAYER_SHELL_EDGE_LEFT,
                                 false);
            gtk_layer_set_anchor(window_.gobj(), GTK_LAYER_SHELL_EDGE_RIGHT,
                                 false);
        }
    }
   ...
   private:
    ...
    void onConfigure(GdkEventConfigure* ev) {
         ...
         * Actual resizing and management of the exclusve zone is handled within
         * the gtk-layer-shell code. This event handler only updates stored size
         * of the window and prints some warnings.
         *
         * Note: forced resizing to a window smaller than required by GTK would
         * not work with gtk-layer-shell.
         */
        if (vertical_) {
...
struct RawSurfaceImpl : public BarSurface, public sigc::trackable {
    RawSurfaceImpl(Gtk::Window& window, struct waybar_output& output)
        ...
        window.signal_realize().connect_notify(
            sigc::mem_fun(*this, &RawSurfaceImpl::onRealize));
        window.signal_map_event().connect_notify(
            sigc::mem_fun(*this, &RawSurfaceImpl::onMap));
        window.signal_configure_event().connect_notify(
        ...
        if (layer_surface_) {
            ...
            if (enable) {
                ...
                if ((anchor_ & VERTICAL_ANCHOR) == VERTICAL_ANCHOR) {
                    zone += width_;
                    zone += (anchor_ & ZWLR_LAYER_SURFACE_V1_ANCHOR_LEFT)
                                ? margins_.right
                                : margins_.left;
                } else {
                    zone += height_;
                    zone += (anchor_ & ZWLR_LAYER_SURFACE_V1_ANCHOR_TOP)
                                ? margins_.bottom
                                : margins_.top;
                }
            }
            spdlog::debug("Set exclusive zone {} for output {}", zone,
                          output_name_);
            zwlr_layer_surface_v1_set_exclusive_zone(layer_surface_.get(),
                                                     zone);
        }
    }
    ...
    void setLayer(bar_layer layer) override {
        ...
        if (layer_surface_) {
            if (zwlr_layer_surface_v1_get_version(layer_surface_.get()) >=
                ...
                spdlog::warn(
                    "Unable to change layer: layer-shell implementation is too "
                    "old");
            }
        }
    }
    ...
    void setMargins(const struct bar_margins& margins) override {
        ...
        // updating already mapped window
        if (layer_surface_) {
            zwlr_layer_surface_v1_set_margin(layer_surface_.get(), margins_.top,
    ...
    void setSize(uint32_t width, uint32_t height) override {
        ...
        // layer_shell.configure handler should update exclusive zone if size
        // changes
   ...
   private:
    ...
    using layer_surface_ptr =
        std::unique_ptr<zwlr_layer_surface_v1,
                        deleter_fn<zwlr_layer_surface_v1_destroy>>;
    ...
    void onMap(GdkEventAny* ev) {
        ...
        zwlr_layer_surface_v1_set_keyboard_interactivity(layer_surface_.get(),
                                                         false);
        zwlr_layer_surface_v1_set_anchor(layer_surface_.get(), anchor_);
        zwlr_layer_surface_v1_set_margin(layer_surface_.get(), margins_.top,
                                         margins_.right, margins_.bottom,
    ...
    void onConfigure(GdkEventConfigure* ev) {
         ...
         * If the size is not set and the window is smaller than requested by
         * GTK, request resize from layer surface.
         */
    ...
    void setSurfaceSize(uint32_t width, uint32_t height) {
        /* If the client is anchored to two opposite edges,
         * layer_surface.configure will return size without margins for the
         * axis. layer_surface.set_size, however, expects size with margins for
         * the anchored axis. This is not specified by wlr-layer-shell and based
         * on actual behavior of sway.
         *
         * If the size for unanchored axis is not set (0), change request to 1
         * to avoid automatic assignment by the compositor.
         */
        if ((anchor_ & VERTICAL_ANCHOR) == VERTICAL_ANCHOR) {
    ...
    static void onSurfaceConfigure(void* data,
    ...
    static void onSurfaceClosed(void* data,
                                struct zwlr_layer_surface_v1* /* surface */) {
        auto o = static_cast<RawSurfaceImpl*>(data);
...
waybar::Bar::Bar(struct waybar_output* w_output, const Json::Value& w_config)
    ...
    if (config["margin-top"].isInt() || config["margin-right"].isInt() ||
        ...
        margins_ = {
            ...
            config["margin-bottom"].isInt() ? config["margin-bottom"].asInt()
                                            : 0,
        ...
        try {
            if (margins.size() == 1) {
                ...
                margins_ = {
                    .top = gaps, .right = gaps, .bottom = gaps, .left = gaps};
            }
            if (margins.size() == 2) {
...
#ifdef HAVE_GTK_LAYER_SHELL
    bool use_gls = config["gtk-layer-shell"].isBool()
                       ? config["gtk-layer-shell"].asBool()
                       : true;
    if (use_gls) {
    ...
    if (spdlog::should_log(spdlog::level::debug)) {
        // Unfortunately, this function isn't in the C++ bindings, so we have to
        // call the C version.
...
void waybar::Bar::onMap(GdkEventAny*) {
    /*
     * Obtain a pointer to the custom layer surface for modules that require it
     * (idle_inhibitor).
     */
        ...
        for (const auto& name : module_list) {
            try {
                ...
                if (ref.compare(0, 6, "group/") == 0 && ref.size() > 6) {
                    auto group_module =
                        new waybar::Group(ref, *this, config[ref]);
...
auto waybar::Bar::setupWidgets() -> void {
    ...
    if (config["fixed-center"].isBool() ? config["fixed-center"].asBool()
                                        : true) {
